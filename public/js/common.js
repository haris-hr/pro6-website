jQuery(function ($) {
	...
	// When opened directly from disk (file://), Chrome/Safari block XHR to local files.
	// This template relies on XHR-based AJAX transitions (jQuery .load), which can leave the UI
	// stuck in a "loading" state. Disable AJAX transitions in that scenario.
	if (window.location && window.location.protocol === 'file:') {
		$('body').addClass('disable-ajaxload');
	}
	...
	window.ScrollEffects = function() {
		...
		if ($('#hero').hasClass('has-image')) {				
			...
			function setHeroProperties() {
				if (document.querySelector("#hero-caption.hero-full-caption")) {
			...
			if( $('.new-hero-title').length > 0 && $('#hero-caption').length > 0 ){
		...
		if( $('.new-hero-title').length > 0 ){
			...
			// Safety check: if heroTitle doesn't exist, skip this block
			if (!heroTitle) {
				// Hero structure changed, skip Flip animation setup
			} else {
			...
			// Safety check: if any required element is missing, skip Flip animation
			if (!heroCaption || !heroCaptionInner || !heroPlaceholder) {
				// Hero structure changed, skip Flip animation setup
			} else {
			...
			} // end safety check for heroCaption/heroCaptionInner/heroPlaceholder
			} // end safety check for heroTitle
		}
		...
		imagesLoaded('body', function() {
			...
			const staticDuration = 60; // Durata statică a animației (viteza implicită) - increased for slower animation
		...
		hasParallax.forEach(function(hParallax) {
			...
			// Safety check: only animate if at least one media element exists
			var mediaElements = [bgImage, bgVideo].filter(function(el) { return el !== null; });
			if (mediaElements.length === 0) return;
			var parallax = gsap.fromTo( mediaElements, {y: '-20%', scale:1.15}, {y: '20%', scale:1, duration: 1, ease:Linear.easeNone});		
		...
		hasMaskFill.forEach(function(hMaskFill) {				
			...
			gsap.to(spanFillMask, { 					
				scrollTrigger: {
					trigger: hMaskFill,
					start: "top 85%",
					// Slow down the fill effect by requiring more scroll distance
					end: () => `+=${hMaskFill.offsetHeight * 5}`,
					// Slightly smoother scrubbing
					scrub: 2,
				},
				duration: 1, 
	...
	window.Core = function() {
		...
		jQuery(document).ready(function(){
			  ...
			  $(window).on('popstate', function() {
				...
				section.load(url+' .cd-main-content > *', function(responseText, textStatus){
					// If the AJAX request fails (common on file://), fall back to normal navigation.
				  ...
				  // load new content and replace <main> content with the new one
				 	...
				 	var clapat_title = responseText.match(/<title[^>]*>([^<]+)<\/title>/)[1];
					...
					var newPageRawHead = responseText.match(/<head[^>]*>([\s\S.]*)<\/head>/i)[0];
				...
				// Ensure hidden classes are removed after AJAX navigation
				$('body').removeClass('hidden').removeClass('hidden-ball');
				
				// Small delay to ensure DOM is fully updated before re-initializing
				setTimeout(function() {
					LoadViaAjax();
				}, 100);
